...
/* Retrieved from OpenRoberta blockly:
 * https://github.com/OpenRoberta/blockly
 *
 * And modified to work with vanilla Blockly.
 */
 ...
 * @param {string} noteFrequency The initial content of the field.
 * @param {Function=}  opt_validator An optional function that is called to
 *    validate any constraints on what the user entered. Takes the new text as
 *    an argument and returns the accepted text or null to abort the change.
 * @extends {Blockly.FieldTextInput}
 * @constructor
 */
Blockly.FieldNote = function(noteFrequency, opt_validator) {
  Blockly.FieldNote.superClass_.constructor.call(this, noteFrequency, opt_validator);
};
...
Blockly.FieldNote.prototype.showEditor_ = function() {
  ...
  this.htmlInput_.setAttribute('disabled', 'true');
  ...
  for (var j = 28; j < 64; j++) {
    ...
    if (black.indexOf(mod) == -1) {
      ...
      var note = Blockly.utils.dom.createSvgElement('rect', {
...
Blockly.FieldNote.prototype.setValue = function(newValue) {
  ...
  if (this.htmlInput_)
    this.htmlInput_.value = Blockly.FieldNote.NOTENAMES[newValue];
